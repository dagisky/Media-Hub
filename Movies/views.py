...
def index(request):
    if 'user' in request.session:
        ...
        paginator = Paginator(movies_list, 12) #the movie size that will display in the index page of each
        try:
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if '-w' in request.POST['search']:
                ...
                if not Q:
                    movies_list=Movie.objects.filter(Watched=False)
                    page = request.GET.get('page', 1)
